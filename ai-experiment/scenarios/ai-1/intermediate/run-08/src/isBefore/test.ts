...
describe("isBefore", () => {
  ...
  it("returns true if only time is different and first date is before", () => {
    const date1 = new Date(1989, 6, 10, 10, 0, 0);
  ...
  it("returns false if only time is different and first date is after", () => {
  ...
  it("accepts date strings as arguments", () => {
    const result = isBefore("1987-02-11T00:00:00.000Z", "1989-07-10T00:00:00.000Z");
    expect(result).toBe(true);
  });
