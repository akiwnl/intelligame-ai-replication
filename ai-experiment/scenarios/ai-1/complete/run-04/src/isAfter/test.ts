...
describe("isAfter", () => {
  // Test case 1: First date is clearly after the second
  ...
  // Test case 2: First date is clearly before the second
  ...
  // Test case 4: Dates are equal but milliseconds differ (first is after)
  it("should return true if the first date is after due to milliseconds", () => {
    const date1 = new Date(2023, 9, 10, 12, 0, 0, 500);
    const date2 = new Date(2023, 9, 10, 12, 0, 0, 0);
    expect(isAfter(date1, date2)).toBe(true);
  });

  // Test case 5: Dates are equal but milliseconds differ (first is before)
  it("should return false if the first date is before due to milliseconds", () => {
  ...
  it("should work with timestamps as input", () => {
    ...
    const date2 = new Date(2023, 9, 10).getTime();
    expect(isAfter(date1, date2)).toBe(true);
  });

  // Test case 8: Input as date strings
  it("should work with date strings as input", () => {
    const date1 = "2023-11-15T12:00:00Z";
    const date2 = "2023-10-10T12:00:00Z";
    expect(isAfter(date1, date2)).toBe(true);
  });
  ...
  // Test case 13: Timezone considerations (toDate handles local timezone)
  it("should compare dates correctly across timezone boundaries (local time)", () => {
    // These dates represent the same UTC instant, but might be different local times
    // However, getTime() gives UTC milliseconds, so comparison should be consistent
    const dateUTC = new Date("2023-10-10T12:00:00.000Z");
    const dateLocal = new Date(dateUTC.getTime()); // Same instant, but treated as local
    const dateLocalAfter = new Date(dateUTC.getTime() + 1); // 1 ms after

    // isAfter(dateLocalAfter, dateLocal) should be true based on timestamps
    expect(isAfter(dateLocalAfter, dateLocal)).toBe(true);
    expect(isAfter(dateLocal, dateLocalAfter)).toBe(false);
    expect(isAfter(dateLocal, dateLocal)).toBe(false);
  });
});
