...
describe("isBefore", () => {
  ...
  // Test case 4: Dates are equal but different time components (first is earlier)
  it("should return true if the first date is before the second due to time components", () => {
    const date1 = new Date(2024, 0, 1, 10, 0, 0); // Jan 1, 2024 10:00:00
    const date2 = new Date(2024, 0, 1, 10, 0, 1); // Jan 1, 2024 10:00:01
    expect(isBefore(date1, date2)).toBe(true);
  });

  // Test case 5: Dates are equal but different time components (first is later)
  it("should return false if the first date is after the second due to time components", () => {
  ...
  it("should work with timestamps as input", () => {
    ...
    const date2 = new Date(2024, 0, 2).getTime();
    expect(isBefore(date1, date2)).toBe(true);
  });
