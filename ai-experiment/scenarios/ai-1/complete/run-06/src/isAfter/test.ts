...
describe("isAfter", () => {
  ...
  it("should work with timestamps as arguments", () => {
    ...
    const timestamp2 = new Date(2024, 0, 5).getTime();
  ...
  it("should work with parsable strings as arguments", () => {
    ...
    const dateString2 = "2024-01-05T12:00:00.000Z";
  ...
  it("should return false if any date argument is an invalid string", () => {
    const validDate = new Date(2024, 0, 10);
    expect(isAfter(validDate, "invalid date")).toBe(false);
    expect(isAfter("invalid date", validDate)).toBe(false);
  ...
  it("should return false if null or undefined are passed as arguments", () => {
    const validDate = new Date(2024, 0, 10);
    // @ts-ignore
    expect(isAfter(validDate, null)).toBe(false);
    // @ts-ignore
    expect(isAfter(null, validDate)).toBe(false);
    // @ts-ignore
    expect(isAfter(validDate, undefined)).toBe(false);
    // @ts-ignore
    expect(isAfter(undefined, validDate)).toBe(false);
    // @ts-ignore
