...
describe("isFuture", () => {
  ...
  it("should return true if the date is in the future relative to the mocked 'now'", () => {
    const futureDate = new Date(2014, 11, 31); // Dec 31, 2014
    expect(isFuture(futureDate)).toBe(true);
  });

  // Test Case 2: Date in the past
  it("should return false if the date is in the past relative to the mocked 'now'", () => {
  ...
  it("should return false if the date is exactly 'now'", () => {
    ...
    expect(isFuture(nowAsDate)).toBe(false);
  });

  // Test Case 4: Date is just after 'now' (milliseconds)
  it("should return true if the date is just one millisecond after 'now'", () => {
    const justAfterNow = new Date(MOCKED_NOW + 1);
    expect(isFuture(justAfterNow)).toBe(true);
  });

  // Test Case 5: Date is just before 'now' (milliseconds)
  it("should return false if the date is just one millisecond before 'now'", () => {
    const justBeforeNow = new Date(MOCKED_NOW - 1);
    expect(isFuture(justBeforeNow)).toBe(false);
  });
  ...
  // Test Case 7: Input as timestamp (future)
  it("should return true when timestamp is in the future", () => {
    const futureTimestamp = new Date(2015, 0, 1).getTime(); // Jan 1, 2015
    expect(isFuture(futureTimestamp)).toBe(true);
  });

  // Test Case 8: Input as timestamp (past)
  it("should return false when timestamp is in the past", () => {
    const pastTimestamp = new Date(2013, 11, 31).getTime(); // Dec 31, 2013
    expect(isFuture(pastTimestamp)).toBe(false);
  });

  // Test Case 9: Input as string (future)
  it("should return true when date string is in the future", () => {
    const futureDateString = "2014-10-06T11:00:00.001Z"; // 1ms after mocked now (in UTC)
    expect(isFuture(futureDateString)).toBe(true);
  });

  // Test Case 10: Input as string (past)
  it("should return false when date string is in the past", () => {
  ...
  it("should return true for a date one day after 'now'", () => {
  ...
  it("should return false for a date one day before 'now'", () => {
    const prevDay = new Date(MOCKED_NOW);
    prevDay.setDate(prevDay.getDate() - 1);
    expect(isFuture(prevDay)).toBe(false);
  });
});
