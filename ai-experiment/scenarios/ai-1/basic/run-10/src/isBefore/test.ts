...
describe("isBefore", () => {
  ...
  it("returns true if the first date is equal to the second date (different time, same day)", () => {
    const date = new Date(1989, 6, 10, 11, 59, 59, 999);
    const dateToCompare = new Date(1989, 6, 10, 12, 0, 0);
    expect(isBefore(date, dateToCompare)).toBe(true);
  });

  it("accepts timestamps", () => {
    const date = new Date(1987, 1, 11).getTime();
    const dateToCompare = new Date(1989, 6, 10).getTime();
    expect(isBefore(date, dateToCompare)).toBe(true);
  });

  it("accepts date strings", () => {
    const date = "1987-02-11T00:00:00.000Z";
    const dateToCompare = "1989-07-10T00:00:00.000Z";
    expect(isBefore(date, dateToCompare)).toBe(true);
  });
